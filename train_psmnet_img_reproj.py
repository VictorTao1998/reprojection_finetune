...
Feature: Train PSMNet Image reprojection
"""
import gc
import os
import argparse
...
# python -m torch.distributed.launch train_psmnet_img_reproj.py --summary-freq 1 --save-freq 1 --logdir ../train_10_14_psmnet_reprojection/debug --debug
# python -m torch.distributed.launch train_psmnet_img_reproj.py --config-file configs/remote_train_steps.yaml --summary-freq 10 --save-freq 100 --logdir ../train_10_14_psmnet_reprojection/debug --debug
# python -m torch.distributed.launch train_psmnet_img_reproj.py --config-file configs/remote_train_primitive_randscenes.yaml --summary-freq 10 --save-freq 100 --logdir ../train_10_14_psmnet_reprojection/debug --debug
...
def train_sample(sample, transformer_model, psmnet_model,
    ...
    img_L_gray_scale = Transforms.Grayscale()(img_L)
    img_R_gray_scale = Transforms.Grayscale()(img_R)
    sim_img_reproj_loss, sim_img_warped, sim_img_reproj_mask = get_reproj_error_patch(
        input_L=img_L_gray_scale,
        input_R=img_R_gray_scale,
        pred_disp_l=sim_pred_disp,
        mask=mask,
        ps=args.ps
    )
    # Backward on sim image reprojection
    sim_loss = loss_psmnet * args.loss_ratio_sim + sim_img_reproj_loss
    if isTrain:
    ...
    img_real_L_gray_scale = Transforms.Grayscale()(img_real_L)
    img_real_R_gray_scale = Transforms.Grayscale()(img_real_R)
    real_img_reproj_loss, real_img_warped, real_img_reproj_mask = get_reproj_error_patch(
        input_L=img_real_L_gray_scale,
        input_R=img_real_R_gray_scale,
        pred_disp_l=real_pred_disp,
        ps=args.ps
    )

    # Backward on real
    real_loss = real_img_reproj_loss * args.loss_ratio_real
    if isTrain:
        transformer_optimizer.zero_grad()
        psmnet_optimizer.zero_grad()
        real_loss.backward()
    ...
    img_output_reproj = {
        'sim_reprojection': {
            'target': img_L_gray_scale, 'warped': sim_img_warped, 'pred_disp': sim_pred_disp, 'mask': sim_img_reproj_mask
        },
        'real_reprojection': {
            'target': img_real_L_gray_scale, 'warped': real_img_warped, 'pred_disp': real_pred_disp, 'mask': real_img_reproj_mask
        }
    }
    ...
    scalar_outputs_psmnet = {
        ...
        'sim_reprojection_loss': sim_img_reproj_loss.item(),
        'real_reprojection_loss': real_img_reproj_loss.item()
    }
