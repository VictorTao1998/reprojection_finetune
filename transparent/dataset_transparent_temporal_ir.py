...
Feature: Load data from transparent dataset, only load simulation dataset
"""
import os
import numpy as np
import random
...
def __get_split_files__(split_file, debug=False, sub=100):
    ...
    return img_L, img_R, img_L_no_ir, img_R_no_ir, img_depth_l, img_depth_r, img_meta


class MessytableTransparentDataset(Dataset):
    def __init__(self, split_file, gaussian_blur=False, color_jitter=False, debug=False, sub=100, test=False):
        """
        :param split_file: Path to the split .txt file, e.g. train.txt
        :param gaussian_blur: Whether apply gaussian blur in data augmentation
        :param color_jitter: Whether apply color jitter in data augmentation
        ...
        self.img_meta = __get_split_files__(split_file, debug, sub)
...
if __name__ == '__main__':
    cdataset = MessytableTransparentDataset(cfg.SPLIT.TRAIN)
