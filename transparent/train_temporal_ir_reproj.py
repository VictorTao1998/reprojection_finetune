...
Feature: Train the temporal ir reproj model on transparent dataset
"""
import gc
import os
import argparse
...
from transparent.dataset_transparent_temporal_ir import MessytableTransparentDataset
...
parser.add_argument('--config-file', type=str, default='./config_transparent.yaml', metavar='FILE', help='Config files')
...
parser.add_argument('--ps', type=int, default=15, help='Patch size of doing patch loss calculation')
...
def train(transformer_model, psmnet_model, transformer_optimizer, psmnet_optimizer,
    ...
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ## Train one sample
            scalar_outputs_psmnet_val, img_outputs_psmnet, img_output_reproj = \
                train_sample(sample, transformer_model, psmnet_model, transformer_optimizer,
                             psmnet_optimizer, isTrain=False)

            ## Save result to tensorboard
            if (not is_distributed) or (dist.get_rank() == 0):
                scalar_outputs_psmnet_val = tensor2float(scalar_outputs_psmnet_val)
                avg_val_scalars_psmnet.update(scalar_outputs_psmnet_val)

        ## Update reprojection images for val
        save_images_grid(summary_writer, 'val_reproj', img_output_reproj, epoch_idx, nrow=4)
        ## Update PSMNet images
        save_images(summary_writer, 'val_psmnet', img_outputs_psmnet, epoch_idx)

        ## Update PSMNet losses
        save_scalars(summary_writer, 'val_psmnet', scalar_outputs_psmnet_val, epoch_idx)
...
if __name__ == '__main__':
    # Obtain dataloader
    train_dataset = MessytableTransparentDataset(cfg.SPLIT.TRAIN, gaussian_blur=args.gaussian_blur,
                                                 color_jitter=args.color_jitter, debug=args.debug, sub=args.sub)
    val_dataset = MessytableTransparentDataset(cfg.SPLIT.VAL, gaussian_blur=args.gaussian_blur,
                                               color_jitter=args.color_jitter, debug=args.debug, sub=10)
