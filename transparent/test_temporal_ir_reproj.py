...
def test(transformer_model, psmnet_model, val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        img_disp_l = F.interpolate(img_disp_l, scale_factor=0.5, mode='nearest',
                             recompute_scale_factor=False)
        img_depth_l = F.interpolate(img_depth_l, scale_factor=0.5, mode='nearest',
                             recompute_scale_factor=False)
        ...
        if args.warp_op:
            ...
            img_disp_r = F.interpolate(img_disp_r, scale_factor=0.5, mode='nearest',
                                       recompute_scale_factor=False)
            img_depth_r = F.interpolate(img_depth_r, scale_factor=0.5, mode='nearest',
                                        recompute_scale_factor=False)
...
def main():
    ...
    test_loader = MessytableTransparentDataset(cfg.SPLIT.TEST, gaussian_blur=False,
                                               color_jitter=False, debug=args.debug,
                                               sub=10, test=True)
