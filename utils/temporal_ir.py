...
parser = argparse.ArgumentParser(description='Extract IR pattern from temporal real images')
parser.add_argument('--split-file', type=str, default='/cephfs/datasets/iccv_pnp/messy-table-dataset/rand_scenes/train_list.txt',
                    metavar='FILE')
parser.add_argument('--data-folder', type=str, default='/cephfs/datasets/iccv_pnp/messy-table-dataset/rand_scenes/rand_scenes')
...
def main():
    ...
    for idx in range(num):
        for direction in ['irL', 'irR']:
            ...
            img_temp = np.concatenate((img_0[:, :, None],
                                       img_1[:, :, None],
                                       img_2[:, :, None],
                                       img_3[:, :, None],
                                       img_4[:, :, None],
            ...
            plt.imsave(os.path.join(args.data_folder, p, f'1024_{direction}_real_temporal.png'), pattern)
            plt.close('all')
