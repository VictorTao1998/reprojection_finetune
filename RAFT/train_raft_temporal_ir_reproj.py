"""
Author: Isabella Liu 1/25/22
Feature: Train ActiveZeroNet with RAFT as backbone
"""
...
from utils.util import setup_logger, weights_init, \
    save_scalars, save_scalars_graph, save_images, save_images_grid, disp_error_img
...
parser = argparse.ArgumentParser(description='IR reprojection with RAFT')
...
parser.add_argument('--loss-ratio-sim', type=float, default=0.01, help='Ratio between loss_psmnet_sim and loss_reprojection_sim')
...
# python -m torch.distributed.launch RAFT/train_raft_temporal_ir_reproj.py --summary-freq 1 --save-freq 1 --logdir ../train_10_14_psmnet_ir_reprojection/debug --debug
# python -m torch.distributed.launch RAFT/train_raft_temporal_ir_reproj.py --config-file ./RAFT/raft_config.yaml --summary-freq 10 --save-freq 100 --logdir /isabella-slow/RAFT/train_1_26_ir_reproj/debug --debug --mixed_precision
...
def train(raft_model, raft_optimizer, raft_scheduler, raft_scaler, TrainImgLoader, ValImgLoader):
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            # Train one sample
            scalar_outputs_raft, img_outputs_raft, img_output_reproj = \
                train_sample(sample, raft_model, raft_optimizer, raft_scheduler, raft_scaler, isTrain=True)
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                ...
                if do_summary:
                    ...
                    save_images(summary_writer, 'train_psmnet', img_outputs_raft, global_step)
                    # Update PSMNet losses
                    scalar_outputs_raft.update({'lr': raft_optimizer.param_groups[0]['lr']})
                    save_scalars(summary_writer, 'train_raft', scalar_outputs_raft, global_step)
...
def train_sample(sample, raft_model, raft_optimizer, raft_scheduler, raft_scaler, isTrain=True):
    if isTrain:
        raft_model.train()
    else:
        raft_model.eval()
    ...
    mask = (disp_gt_l < cfg.ARGS.MAX_DISP) * (disp_gt_l > 0)  # Note in training we do not exclude bg [bs, 1, H, W]

    if isTrain:
        flow_pred = raft_model(img_L, img_R, iters=args.train_iters)  # Note here flow_pred is a list
    ...
    sim_loss = loss_raft * args.loss_ratio_sim + sim_ir_reproj_loss
    ...
    scalar_outputs_raft = {'loss': loss_raft.item(),
...
if __name__ == '__main__':
    ...
    raft_optimizer = torch.optim.AdamW(raft_model.parameters(), lr=args.lr, weight_decay=args.wdecay, eps=1e-8)
    raft_scheduler = torch.optim.lr_scheduler.OneCycleLR(raft_optimizer, args.lr, args.num_steps+100, pct_start=0.01,
    ...
    # Start training
    train(raft_model, raft_optimizer, raft_scheduler, raft_scaler, TrainImgLoader, ValImgLoader)