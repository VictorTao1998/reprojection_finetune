...
class MessytableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Compute flow map from disparity
        img_flow_l = np.stack([-img_disp_l, np.zeros_like(img_disp_l)], axis=-1)  # [H, W, 2]
        img_flow_r = np.stack([-img_disp_r, np.zeros_like(img_disp_l)], axis=-1)
        ...
        img_flow_l = img_flow_l[2 * x: 2 * (x + th), 2 * y: 2 * (y + tw)]
        img_flow_r = img_flow_r[2 * x: 2 * (x + th), 2 * y: 2 * (y + tw)]
        ...
        item['img_flow_l'] = torch.tensor(img_flow_l, dtype=torch.float32).permute(2, 0, 1)  # [bs, 2, H, W]
        item['img_flow_r'] = torch.tensor(img_flow_r, dtype=torch.float32).permute(2, 0, 1)  # [bs, 2, H, W]
