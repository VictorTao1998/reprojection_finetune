...
from utils.reprojection import get_reprojection_error, get_reprojection_error_old, get_reprojection_error_diff_ratio
...
def train(transformer_model, psmnet_model, transformer_optimizer, psmnet_optimizer,
    ...
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            sim_ir_reproj_output, real_ir_reproj_output, \
            sim_ir_reproj_loss_dict, real_ir_reproj_loss_dict = \
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                ...
                if do_summary:
                    ...
                    save_images_grid(summary_writer, 'train_sim_reproj', sim_ir_reproj_output, global_step, nrow=4)
                    save_images_grid(summary_writer, 'train_real_reproj', real_ir_reproj_output, global_step, nrow=4)
                    # Update reprojection losses
                    save_scalars(summary_writer, 'train_sim_reproj', sim_ir_reproj_loss_dict, global_step)
                    save_scalars(summary_writer, 'train_real_reproj', real_ir_reproj_loss_dict, global_step)
...
def train_sample(sample, transformer_model, psmnet_model,
    ...
    disp_gt_l = F.interpolate(disp_gt_l, scale_factor=0.5, mode='bilinear',
                             recompute_scale_factor=False)  # [bs, 1, H, W]
    depth_gt = F.interpolate(depth_gt, scale_factor=0.5, mode='bilinear',
                             recompute_scale_factor=False)  # [bs, 1, H, W]

    if args.warp_op:
        img_disp_r = sample['img_disp_r'].to(cuda_device)
        img_disp_r = F.interpolate(img_disp_r, scale_factor=0.5, mode='bilinear',
    ...
    sim_ir_reproj_loss, sim_ir_reproj_output, sim_ir_reproj_loss_dict\
        = get_reprojection_error_diff_ratio(img_L_ir_pattern, img_R_ir_pattern, sim_pred_disp, mask)
    ...
    real_ir_reproj_loss, real_ir_reproj_output, real_ir_reproj_loss_dict\
        = get_reprojection_error_diff_ratio(img_real_L_ir_pattern, img_real_R_ir_pattern, real_pred_disp)
    ...
    if is_distributed:
        ...
        sim_ir_reproj_loss_dict = reduce_scalar_outputs(sim_ir_reproj_loss_dict, cuda_device)
        real_ir_reproj_loss_dict = reduce_scalar_outputs(real_ir_reproj_loss_dict, cuda_device)
