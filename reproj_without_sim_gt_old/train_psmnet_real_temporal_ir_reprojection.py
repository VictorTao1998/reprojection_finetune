...
from utils.reprojection import get_reprojection_error, get_reprojection_error_old, get_reproj_error_patch
...
parser.add_argument('--ps', type=int, default=5, help='Patch size of doing patch loss calculation')
...
def train_sample(sample, transformer_model, psmnet_model,
    ...
    real_ir_reproj_loss, real_ir_warped, real_ir_reproj_mask = get_reproj_error_patch(img_real_L_ir_pattern, img_real_R_ir_pattern, real_pred_disp, ps=args.ps)
    ...
    if is_distributed:
        scalar_outputs_reproj = reduce_scalar_outputs(scalar_outputs_reproj, cuda_device)
    return scalar_outputs_reproj, img_output_reproj
