...
class MessytableSequenceDataset(Dataset):
    def __init__(self, seq_folder, idx_s=0, idx_e=100, debug=False, sub=10):
        ...
        """
        self.img_L_real, self.img_R_real, self.img_real_realsense \
            = self.__get_split_files__(seq_folder, idx_s=idx_s, idx_e=idx_e, debug=debug, sub=sub)
    ...
    def __get_split_files__(seq_folder, idx_s=0, idx_e=100, debug=False, sub=10):
        ...
        img_L_real = [os.path.join(seq_folder, '1024_irL_real_' + s + '.png') for s in suffix_list]
        img_R_real = [os.path.join(seq_folder, '1024_irR_real_' + s + '.png') for s in suffix_list]
        img_real_realsense = [os.path.join(seq_folder, '1024_depth_real_' + s + '.png') for s in suffix_list]
    ...
    def __getitem__(self, idx):
        ...
        item['focal_length'] = torch.tensor(self.focal_length, dtype=torch.float32).unsqueeze(0).unsqueeze(0).unsqueeze(0)
        item['baseline'] = torch.tensor(self.baseline, dtype=torch.float32).unsqueeze(0).unsqueeze(0).unsqueeze(0)
...
def get_seq_loader(seq_folder, idx_s=0, idx_e=100, debug=False, sub=10):
    ...
    :return: dataloader
    """
    messytable_dataset = MessytableSequenceDataset(seq_folder=seq_folder, idx_s=idx_s, idx_e=idx_e, debug=debug, sub=sub)
