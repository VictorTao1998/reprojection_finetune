...
parser.add_argument('--output', type=str, default='../1_25_sequence', help='Path to output folder')
...
# python test_sequence.py --model /code/models/model_4.pth --onreal --exclude-bg --exclude-zeros
...
def test(transformer_model, psmnet_model, val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        pred_depth_np = pred_depth_np[:, 30:]
        ...
        realsense_depth_np = realsense_depth_np[:, 30:]
        realsense_depth_np = visualize_depth(realsense_depth_np)

        # Save images
        cv2.imwrite(os.path.join(log_dir, 'pred_depth', f'{iteration:04}.png'), pred_depth_np)
        cv2.imwrite(os.path.join(log_dir, 'realsense_depth', f'{iteration:04}.png'), realsense_depth_np)
...
def main():
    ...
    # val_loader = get_seq_loader('/code/supp_video/3/', idx_s=0, idx_e=254, debug=args.debug, sub=10)
