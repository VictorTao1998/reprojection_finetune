...
class RayRegression(nn.Module):
    def __init__(self):
        super(RayRegression, self).__init__()
    def forward(self, x, disp_candidate):
        alpha_softmax = F.softmax(x, dim=1)
        alpha = 1. - torch.exp(-x)
        B,D,N,W = alpha.shape
        #print(torch.cat([torch.ones(B,1,N,W).to(alpha.device), 1. - alpha + 1e-10], 1).shape)
        T = torch.cumprod(torch.cat([torch.ones(B,1,N,W).to(alpha.device), 1. - alpha + 1e-10], 1), 1)[:, :-1,:,:]
        #print(T.shape)
        weight = alpha * T
        #weight = torch.softmax(weight, 1)
        #print(weight.shape)
        #print(disp_candidate)
        #print(weight.shape, disp_candidate.shape)
        #print(disp_candidate.shape, weight.shape, torch.sum(weight, 1))
        out = torch.sum(weight*disp_candidate, 1, keepdim=True)
        return out
