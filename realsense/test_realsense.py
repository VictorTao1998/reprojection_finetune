...
parser.add_argument('--config', type=str, default='./configs/local_test.yaml',
                    metavar='FILE', help='Config files')
parser.add_argument('--output', type=str, default='../testing_output_11_9_realsense', help='Path to output folder')
...
parser.add_argument('--exclude-zeros', action='store_true', default=False, help='Whether count zeros in pred depth')
...
def test(val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        if args.exclude_bg:
            ...
            mask = (img_disp_l < cfg.ARGS.MAX_DISP) * (img_disp_l > 0) * (pred_depth > 0) * img_ground_mask * robot_mask if args.exclude_zeros else \
                (img_disp_l < cfg.ARGS.MAX_DISP) * (img_disp_l > 0) * img_ground_mask * robot_mask
        else:
            mask = (img_disp_l < cfg.ARGS.MAX_DISP) * (img_disp_l > 0) * (pred_depth > 0) * robot_mask if args.exclude_zeros else \
                (img_disp_l < cfg.ARGS.MAX_DISP) * (img_disp_l > 0) * robot_mask
